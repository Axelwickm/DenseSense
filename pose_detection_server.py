...
shared_mem = posix_ipc.SharedMemory("/densepose_mem")
mapfile = mmap.mmap(shared_mem.fd, shared_mem.size)
shared_mem.close_fd()

shared_mem = posix_ipc.SharedMemory("/densepose_debug_mem")
debug_mapfile = mmap.mmap(shared_mem.fd, shared_mem.size)
shared_mem.close_fd()
...
def trainingLoop(PDS):
    global algorithms, db, debug_mapfile, server
    ...
    statistics = ""
    for i in xrange(0, iterations): # Lol, don't make this "range". 1 hour debugging...
        epoch = 0
        imageInEpoch = 0
        for alg in algorithms:
            if PDS.DP is None: # FIXME: this shouldn't be here
                PDS.DP = Algorithms.Densepose()
            if PDS.PE is None:
                PDS.PE = Algorithms.People_Extractor("/shared/bodyMetrics.json")
...
def white_balance(img): # TODO: make this an algortihm
...
# Websocket handling
class PoseDetectorServer(WebSocket):
        is_processing = False
...
class WebSocketThread(threading.Thread):
    def __init__(self, port):
        ...
        self.server = SimpleWebSocketServer('', port, PoseDetectorServer)
    
    def start(self):
        super(WebSocketThread, self).start()
        ...
        for client in clients:
            ...
            while client.sendq:
                try:
                    ...
                    print("The pop from empty queue problem")

server = WebSocketThread(2043)
server.start()